...
serve(async (req) => {
  ...
  try {
    ...
    // ── 6. Resolution history (last 5 from same client) ──
...
Responda APENAS em JSON válido, sem markdown, sem explicações fora do JSON:
{
  ...
  "suggested_resolution": "Sugestão concreta de como resolver, baseada no contexto disponível, em 3-5 linhas"
    ...
    const aiRes = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content: "Analise este ticket e gere o JSON com problem_summary e suggested_resolution." },
        ],
        max_tokens: 1024,
        temperature: 0.3,
      }),
    ...
    if (!aiRes.ok) {
      ...
      console.error("AI Gateway error status:", errStatus);
      if (errStatus === 429) {
    ...
    if (!problemSummary || !suggestedResolution) {
      // If parse failed, use the raw content as problem_summary
