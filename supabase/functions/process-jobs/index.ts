...
async function handleClassifyBatch(
  ...
  const distinctConvIds: string[] = [...new Set<string>((convRows ?? []).map((r: { conversation_id: string }) => r.conversation_id).filter((id: string | null): id is string => typeof id === 'string' && id.length > 0))];
