DROP FUNCTION IF EXISTS public.get_tech_dashboard_metrics(INT, UUID, UUID);
...
BEGIN
  ...
  col_time_agg AS (
    SELECT COALESCE(json_agg(json_build_object('column_id', column_id, 'column_name', column_name, 'avg_days', avg_days) ORDER BY position), '[]'::json) AS data
    FROM col_time_raw
  ),
  person_wip AS (
...
COMMENT ON FUNCTION public.get_tech_dashboard_metrics IS
  'Dashboard TECH v2 — CTEs isolados, sem agregações aninhadas. TODO: enum demand_event_type não tem reopened.';